...
function aggregateReactions(raw) {
  const map = {};
  raw.forEach((r) => { map[r.emoji] = (map[r.emoji] ?? 0) + 1; });
    ...
    <div style={{ minHeight:"100%", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column" }}>
        ...
        {posts.map((post) => {
          ...
          const itemsByCategory = Object.fromEntries(post.post_items.map((i) => [i.category, i]));
          return (
                  ...
                  {CATEGORIES.map((c) => {
                    ...
                    return (
                      <div key={c.key} style={{ aspectRatio:"1", position:"relative", overflow:"hidden", background:item?`${CELL_TEXTURE}, ${gradient}`:"#EAE2D2", boxShadow:item?"inset 0 0 30px rgba(43,43,40,0.14)":"none" }}>
                        {item?.signedUrl ? <img src={item.signedUrl} alt={c.label} className="film-photo" style={{ width:"100%", height:"100%", objectFit:"cover" }}/> : null}

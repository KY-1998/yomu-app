...
function LandingPageInner() {
  ...
  // 招待リンクから来た場合はコードを抽出して表示
  const inviteMatch = next.match(/^\/join\/([A-Z0-9]+)$/);
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        {inviteCode ? (
          <div className="flex flex-col items-center gap-1 text-center">
            <p className="text-sm text-foreground font-medium">友達から招待が届いています</p>
            <p className="font-mono text-xs tracking-widest text-accent">{inviteCode}</p>
            <p className="text-xs text-muted">ログインして友達になりましょう</p>
          </div>
        ) : (
...
export default function LandingPage() {
  return (
    <Suspense fallback={<div className="flex min-h-dvh items-center justify-center text-sm text-muted">読み込み中...</div>}>
      <LandingPageInner />
    </Suspense>
  );
}
